...
public class IfExpression : ASTNode, PrimaryExpression {
  public enum ElseClause {
    case `else`(CodeBlock)
    indirect case elseif(IfExpression)
  }
  ...
  override public var textDescription: String {
    return "if \(conditionList.textDescription) \(codeBlock.textDescription) \(elseClause.textDescription)"
  }
}

extension IfExpression.ElseClause : ASTTextRepresentable {
  public var textDescription: String {
    switch self {
    case .else(let codeBlock):
      return "else \(codeBlock.textDescription)"
    case .elseif(let ifExpr):
      return "else \(ifExpr.textDescription)"
    }
  }
}
