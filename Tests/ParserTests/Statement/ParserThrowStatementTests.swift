...
class ParserThrowStatementTests: XCTestCase {
  func testThrow() {
    parseStatementAndTest("throw foo", "throw foo", testClosure: { stmt in
      guard let throwStmt = stmt as? ThrowStatement else {
        XCTFail("Failed in parsing a throw statement.")
        return
      }
      let expr = throwStmt.expression
